// REPL driver for the Winery POS Electron app.
// Run on a machine with a real X display (DISPLAY=:0 here) or under xvfb-run.
...
const COMMANDS = {
  ...
  async logs() {
    // no-op placeholder; console listener attached at launch time below
    console.log('(see live console.* lines above)');
  },
...
console.log('winery-pos driver — "help" for commands, "launch" to start');
rl.prompt();
